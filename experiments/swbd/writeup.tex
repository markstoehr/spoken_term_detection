...
cat swb_ms98_transcriptions/*/*/*-word.text | local/collect_word_stats.py -w data/local/words.txt -o data/local/word_stats
\end{verbatim}
...
\begin{verbatim}
sed 's:\-:\ :' DetailedNewAFKSData_description/train.reference.5000.wavlist.txt | awk '{ print $1 }' | sed 's/[AB]/.sph/' | sed 's:sw:/var/tmp/stoehr/switchboard/swbI_release2/ALL/sw0:' | sort | uniq > $dir/train.sph
\end{verbatim}
and then we get the lines not shared between the two files to get the
test set
\begin{verbatim}
diff --suppress-common-lines $dir/sph.flist $dir/train.sph | grep '^<' | sed 's/^< //' > test.sph
\end{verbatim}
